...
`c` commits without leaving the list. Tick the files you want (everything is selected by default, `Space` toggles), type a message, and `Enter` commits. If the selection looks like one side of a renamed folder, you're asked whether to include the other side so git records the rename.
...
`e` opens a small editor to **hand-edit the current hunk**, seeded with both sides so nothing is lost. `Ctrl+S` **writes the file to disk immediately** — hunks you haven't decided yet keep their conflict markers, so the file stays valid and your edit survives moving to another file and back. `Esc` discards the edit. The resolver then shows the text your edit will write, under its own `YOUR EDIT` heading.

`⇧E` opens the **whole file** instead, conflict markers and all, for the fixes per-hunk choices can't express (interleaving both sides, fixing an import list, deleting a stray line). Choices you have already made are written to disk first, so you edit the file as it really stands; the markers are highlighted and each side keeps its resolver colour. `Ctrl+S` saves and re-reads the file, so any hunks the edit settled drop out of the list; `Esc` discards.
